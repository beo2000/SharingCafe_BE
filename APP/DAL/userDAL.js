...
export async function getUserByFilterSetting(
  ...
  WHERE
    ...
    AND CASE WHEN (select by_interest from user_filter_matching) = TRUE THEN (EXISTS (SELECT 1 FROM user_interest ui WHERE ui.user_id = u.user_id AND ui.interest_id IN (${
      interest_ids
        ? `${interest_ids
            .split(';')
            .map((id) => `'${id}'`)
